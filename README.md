...
所有冻结/解冻入口（单应用操作、`FREEZE_TAG`、`FREEZE_ALL` / `UNFREEZE_ALL`、首页 FAB 与菜单、
多选、自动冻结、延迟任务、启动应用）都按各应用所属分组逐个解析模式；全局工作模式仅作为未配置分组的
默认 fallback。

已知限制：解冻只走指定模式（无跨模式兜底），因此用一次性 `mode` 覆盖冻上的、且与分组模式/全局都对不上的
应用，需用同样的覆盖参数解冻。

### 冻结后杀进程（fork 扩展）

冻结成功后（`stop` 类本来就是杀、Island 跳过），Hail 会静默补杀该应用进程，不留残留。
通道顺序：Shizuku → ROOT。（Dhizuku 通道已实测无效：其起的进程是 Dhizuku 自身 uid，
无 `FORCE_STOP_PACKAGES` 权限，故弃用。）
失败静默忽略：不返回结果、不弹任何提示。

### 配置备份与恢复（fork 扩展，取自上游 #425 并适配）

设置页新增「配置管理」：备份 / 恢复 / 重置。备份为 `.hail` 文件（zip），包含全局工作模式、
全部偏好、首页应用（含置顶/白名单/分组归属）与分组（含独立冻结模式），**不含**应用实际冻结状态。

- 恢复前会校验并展示确认页（设备名、版本、应用/分组数；版本不一致或带冻结改模式时警告），校验失败拒绝写入。
- 恢复会整体替换当前配置；重置清空配置（均不改变应用实际状态）。
- 换签名重装导致数据丢失时，用备份恢复分组与模式配置。
