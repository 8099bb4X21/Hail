    ...
    private fun validateTags(
        ...
        runCatching {
            ...
            for (i in 0 until tags.length()) {
                ...
                // Fork extension: per-tag working mode must be a known non-default mode.
                val mode = tag.optString(HailData.KEY_MODE, null)?.ifEmpty { null }
                if (mode != null
                    && (mode == HailData.MODE_DEFAULT || mode !in HailData.WORKING_MODE_VALUES)
                ) {
                    errors += BackupValidationError.TagInvalidMode(
                        index = i,
                        mode = mode
                    )
                }
